...
import type { GetUpdatesResp, MessageItem, WeixinMessage } from "./types.js";
...
async function apiPost<T>(
  ...
  try {
    ...
    if (!res.ok) throw new Error(`API ${endpoint} responded ${res.status}: ${text}`);
...
export async function sendTextMessage(
  ...
  const items: MessageItem[] = text ? [{ type: MessageItemType.TEXT, text_item: { text } }] : [];
